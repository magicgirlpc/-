...
- Vertical continuity: the reference creates two complete rounds of project planes and loads every texture, which keeps the cylinder populated above and below the center. The local DOM version keeps one lightweight card round, but calculates the required image window from viewport height, card height and vertical spacing. It loads the center immediately, then staggers the remaining visible upper/lower covers at low priority. Distant visible cards retain enough opacity and reduced blur to read as a continuing helix rather than empty placeholders.
