...
- Once the visitor enters, the URL carries `entered=1` together with the current `view`. If iOS discards and reloads the tab under memory pressure, the portfolio restores directly to the saved gallery rather than showing the welcome screen again. First-time URLs without this marker still show the welcome experience.
- Chrome reports whether a menu/panel is covering the gallery; this pauses background spiral work without changing the visible desktop or mobile layout.
